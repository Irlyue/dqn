...
from tf_util import is_placeholder, get_latest_session, get_variable_name
...
class Function(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        for input_ in self.inputs[len(args):]:
            input_name = get_variable_name(input_)
        ...
        # run it
        results = get_latest_session().run(self.outputs_update, feed_dict)[::-1]
        if self.check_nan:
